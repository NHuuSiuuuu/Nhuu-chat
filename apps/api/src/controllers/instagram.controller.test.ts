...
describe("Instagram controller", () => {
  ...
  it("returns safe owner connections and filters staff by channel grant", async () => {
    accounts.list.mockResolvedValue([{ id: "1", instagramUserId: "123" }, { id: "2", instagramUserId: "456" }]);
    ...
    expect(response.json).toHaveBeenCalledWith({ connections: [{ id: "2", instagramUserId: "456" }] });
  });

  it("redirects successful and cancelled callbacks through a fixed frontend path", async () => {
    oauth.finish.mockResolvedValue({ id: "1" });
    ...
    expect(success.redirect).toHaveBeenCalledWith(302, "https://app.example.com/dashboard?instagram_oauth=success");
