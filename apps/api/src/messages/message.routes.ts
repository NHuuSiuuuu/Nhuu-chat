...
messageRouter.post("/send", requireRole(...inboxAccessRoles), async (req, res, next) => {
  try {
    ...
    emitInboxEvent("chat:conversation_updated", conversation.ownerId ? String(conversation.ownerId) : null, toConversation({ ...conversation, lastMessageAt: new Date(), lastMessageSnippet: content }));
