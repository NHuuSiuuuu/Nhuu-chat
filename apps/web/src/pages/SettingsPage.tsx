...
function QuickReplyModal({ reply, isSaving, onClose, onSave }: { reply: QuickReplyContract | null; isSaving: boolean; onClose: () => void; onSave: (reply: QuickReplyDraft) => Promise<boolean> }) {
...
function QuickReplySettings({ token, refresh }: { token: string; refresh?: () => Promise<string | null> }) {
  ...
  async function loadQuickReplies() {
    ...
    try {
      ...
      setError("Không thể tải danh sách trả lời nhanh");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function openAddQuickReplyModal() { setEditingQuickReply(null); setError(null); setIsAddQuickReplyModalOpen(true); }
  function openEditQuickReplyModal(reply: QuickReplyContract) { setEditingQuickReply(reply); setError(null); setIsAddQuickReplyModalOpen(true); }
  ...
  async function saveQuickReply(reply: QuickReplyDraft) {
    ...
    try {
      ...
      setError(reply.attachment ? "Không thể tải ảnh đính kèm" : editingQuickReply ? "Không thể cập nhật mẫu trả lời nhanh" : "Không thể thêm mẫu trả lời nhanh");
      return false;
    } finally {
      setIsSaving(false);
    }
  ...
  async function removeQuickReply(reply: QuickReplyContract) {
    ...
    try {
      ...
      setError("Không thể xóa mẫu trả lời nhanh");
    } finally {
      setDeletingId(null);
    }
  }
