...
export function InboxPage({ token, refresh, platform, onBack, onLogoClick, onNavigate, user, onLogout, onProfile }: { token: string; refresh?: () => Promise<string | null>; platform?: string; onBack?: () => void; onLogoClick?: () => void; onNavigate?: (item: "Hội thoại" | "Đơn hàng" | "Bài viết" | "Thống kê" | "Cài đặt") => void; user?: InboxAccount | null; onLogout?: () => void; onProfile?: () => void }) {
  ...
  // Gán hội thoại cho nhân viên hiện tại trước khi phản hồi để backend khóa bot trong thời gian bàn giao.
  async function takeOverActiveConversation() {
    if (!activeId || !user?.id || isTakingOver) return;
    setIsTakingOver(true);
    setTakeoverError(null);
    try {
      const result = await apiRequest<ConversationContract>(API_URL, `/api/v1/conversations/${activeId}/assignment`, token, { method: "PATCH", body: JSON.stringify({ assignedAgentId: user.id }) }, refresh);
      setConversations((current) => {
      ...
      setTakeoverError("Không thể tiếp quản hội thoại.");
    } finally {
