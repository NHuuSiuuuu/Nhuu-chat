...
import { appendUniqueMessage, upsertConversation } from "../state/inbox-realtime.js";
...
export function InboxPage({ token, refresh }: { token: string; refresh?: () => Promise<string | null> }) {
  ...
  useEffect(() => { if (!activeId) { setMessages([]); return; } void apiRequest<{ messages: ChatMessageContract[] }>(API_URL, `/api/v1/conversations/${activeId}/messages`, token, {}, refresh).then((result) => setMessages(result.messages)); }, [activeId, token, refresh]);
  useEffect(() => {
    ...
    socket.on(chatEvents.conversationUpdated, (conversation: ConversationContract) => setConversations((current) => upsertConversation(current, conversation)));
    joinActiveRoom();
