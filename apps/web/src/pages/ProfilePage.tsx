...
function Avatar({ name, avatarUrl, onChange }: { name: string; avatarUrl?: string | null; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) {
  ...
  return <div className="relative size-16 shrink-0"><div className="grid size-16 place-items-center overflow-hidden rounded-full bg-sky-100 text-xl font-bold text-sky-700">{avatarUrl ? <img className="size-full object-cover" src={avatarUrl} alt={`Avatar ${name}`} /> : initial}</div><label className="absolute -bottom-1 -right-1 grid size-7 cursor-pointer place-items-center rounded-full border-2 border-white bg-sky-600 text-white shadow-sm transition hover:bg-sky-700" aria-label="Đổi ảnh đại diện" title="Đổi ảnh đại diện"><InboxIcon name="camera" size={14} /><input className="sr-only" type="file" accept="image/*" onChange={onChange} aria-label="Đổi ảnh đại diện" /></label></div>;
}

export function ProfilePage({ token, refresh, user, onLogoClick, onNavigate, onLogout, onProfile }: { token: string; refresh?: () => Promise<string | null>; user: DashboardAccount; onLogoClick?: () => void; onNavigate?: (item: "Hội thoại" | "Đơn hàng" | "Bài viết" | "Thống kê" | "Cài đặt") => void; onLogout?: () => void; onProfile?: () => void }) {
