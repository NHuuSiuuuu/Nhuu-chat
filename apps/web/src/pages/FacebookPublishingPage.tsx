...
export function FacebookPublishingPage({ onBack }: { onBack?: () => void }) {
  ...
  function selectImage(file: File | undefined) {
    if (!file) return;
    if (!IMAGE_TYPES.includes(file.type) || file.size > MAX_IMAGE_BYTES) { setError("Ảnh phải là JPEG, PNG hoặc WebP và không quá 5 MiB."); return; }
  ...
  async function runAction(action: () => Promise<unknown>) {
    ...
    try { await action(); await loadPosts(); } catch (requestError) { setActionError(errorMessage(requestError)); } finally { setBusy(false); }
  }
