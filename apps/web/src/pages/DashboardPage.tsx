...
export function DashboardPage({ token, refresh, onOpenInbox, onLogoClick, onNavigate, user, onLogout, onProfile, settingsSubmenuItems, onSettingsSubmenuNavigate }: { token: string; refresh?: () => Promise<string | null>; onOpenInbox: (platform?: DashboardConnectedAccount["id"]) => void; onLogoClick?: () => void; onNavigate?: (item: "Hộp thư" | "Đơn hàng" | "Bài viết" | "Thống kê" | "Cài đặt") => void; user?: DashboardAccount | null; onLogout?: () => void; onProfile?: () => void; settingsSubmenuItems?: readonly string[]; onSettingsSubmenuNavigate?: (item: string) => void }) {
  ...
  return <main className="min-h-screen bg-[#f2f5f9] pt-16 text-[#273348] max-[700px]:px-[34px] max-[700px]:pt-0" aria-labelledby="dashboard-title">
    <DashboardTopbar onLogoClick={onLogoClick} onNavigate={onNavigate} user={user} onLogout={onLogout} onProfile={onProfile} settingsSubmenuItems={settingsSubmenuItems} onSettingsSubmenuNavigate={onSettingsSubmenuNavigate} />
    <div className="mx-auto w-full max-w-[954px] max-[700px]:mt-[34px]">
      <header className="mb-2 flex items-end justify-between gap-6 rounded-[14px] bg-white px-[17px] pb-4 pt-[19px] max-[700px]:items-stretch max-[700px]:flex-col">
        ...
        <div className="flex items-center gap-[9px] max-[700px]:justify-end"><button className="grid h-10 w-10 place-items-center rounded-[9px] border border-[#e0e6ee] bg-white text-[#69778b] focus-visible:outline-2 focus-visible:outline-[#86b8ff] focus-visible:outline-offset-2 disabled:cursor-wait disabled:opacity-60" type="button" aria-label="Làm mới" onClick={() => void reloadStatus()} disabled={isReloading}><InboxIcon name="refresh" size={18} className={isReloading ? "animate-spin" : ""} /></button><button className="h-10 rounded-[9px] border border-[#e0e6ee] bg-[#e9edf3] px-4 text-[13px] font-bold text-[#354258] focus-visible:outline-2 focus-visible:outline-[#86b8ff] focus-visible:outline-offset-2 max-[700px]:flex-1" type="button" onClick={openModal}><span className="text-lg">＋</span> Kết nối</button><button className="flex h-10 items-center gap-2 whitespace-nowrap rounded-[9px] border border-[#e0e6ee] bg-white px-4 text-[13px] font-bold text-[#354258] focus-visible:outline-2 focus-visible:outline-[#86b8ff] focus-visible:outline-offset-2 max-[700px]:flex-1" type="button" onClick={openMergeModal}><InboxIcon name="layers" size={17} /> Gộp trang</button></div>
      </header>
