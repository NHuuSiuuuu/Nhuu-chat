...
export function TelegramPersonalPage({ token, refresh, onBack }: { token: string; refresh?: () => Promise<string | null>; onBack: () => void }) {
  ...
  return <main aria-labelledby="telegram-title"><button className="cursor-pointer transition-opacity hover:opacity-80" onClick={onBack}>← Dashboard</button><h1 id="telegram-title">Kết nối Telegram cá nhân</h1><p>Mở Telegram trên điện thoại, vào Settings → Devices → Link Desktop Device rồi quét mã QR.</p>{!qr && <button className="cursor-pointer transition-opacity hover:opacity-80" onClick={() => void start()}>Tạo mã QR</button>}{image && <figure><img src={image} alt="Mã QR đăng nhập Telegram" /><figcaption>Mã QR hết hạn nhanh; hãy quét ngay.</figcaption></figure>}{qr?.status === "password_required" && <><form onSubmit={submitPassword}><p>Telegram yêu cầu mật khẩu xác minh hai bước{qr.passwordHint ? ` (${qr.passwordHint})` : ""}.</p><label>Mật khẩu Telegram<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required autoComplete="current-password" /></label>{qr.error === "TELEGRAM_2FA_PASSWORD_INVALID" && <p role="alert">Mật khẩu Telegram không đúng, hãy thử lại.</p>}<button className="cursor-pointer disabled:cursor-not-allowed" type="submit" disabled={submittingPassword}>{submittingPassword ? "Đang xác minh..." : "Xác minh mật khẩu"}</button></form><button className="cursor-pointer transition-opacity hover:opacity-80" type="button" onClick={() => void start()}>Tạo phiên QR mới</button></>}{qr?.status === "connected" && <p role="status">Đã kết nối Telegram thành công.</p>}{qr?.status === "failed" && <p role="alert">{qr.error ?? "Kết nối Telegram thất bại"}</p>}{error && <p role="alert">{error}</p>}</main>;
}
