...
export function ConversationAvatar({ name, avatarUrl, isGroup, size = "size-[42px]" }: { name: string; avatarUrl?: string; isGroup?: boolean; size?: string }) {
  ...
  if (normalizedAvatarUrl && !avatarFailed) return <img className={`${size} shrink-0 rounded-full object-cover`} src={normalizedAvatarUrl} alt={`Avatar ${name}`} onError={() => setAvatarFailed(true)} />;
