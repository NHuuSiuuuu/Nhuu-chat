...
function renderMessageAttachments(message: ChatMessageContract): React.ReactNode {
  return message.attachments?.map((attachment) => attachment.mimeType.startsWith("image/")
    ? <a className="mb-2 block overflow-hidden rounded-lg" href={attachment.url} target="_blank" rel="noreferrer" key={attachment.url} aria-label={`Mở hình ảnh ${attachment.fileName ?? "đính kèm"}`}><img className="max-h-72 max-w-full object-contain" src={attachment.url} alt={attachment.fileName ?? "Hình ảnh đính kèm"} /></a>
    : <a className="mb-2 flex items-center gap-2 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-sky-700 underline" href={attachment.url} target="_blank" rel="noreferrer" key={attachment.url} aria-label={`Tải tệp đính kèm ${attachment.fileName ?? ""}`}><InboxIcon name="file" size={17} /><span className="min-w-0 truncate">{attachment.fileName ?? "Tệp đính kèm"}</span></a>);
}
...
export function ChatWindow({ conversation, messages, onSend, onRetryMessage, quickReplies, onOpenConversationList, isCustomerTyping = false, aiSuggestions, isAiSuggestionsLoading = false, aiSuggestionsError, onRefreshAiSuggestions, aiSuggestionsEnabled = true, availableTags = [], onTagsChange, draft = "", onDraftChange, onToggleBot, isTogglingBot = false, toggleBotError }: { conversation: ConversationContract | null; messages: ChatMessageContract[]; onSend: (content: ComposerSendPayload) => Promise<boolean>; onRetryMessage?: (messageId: string) => void; quickReplies: QuickReplyContract[]; onOpenConversationList?: () => void; isCustomerTyping?: boolean; aiSuggestions?: string[] | null; isAiSuggestionsLoading?: boolean; aiSuggestionsError?: string | null; onRefreshAiSuggestions?: () => void; aiSuggestionsEnabled?: boolean; availableTags?: ConversationTagContract[]; onTagsChange?: (tags: ConversationTagContract[]) => Promise<void>; draft?: string; onDraftChange?: (content: string) => void; onToggleBot?: () => Promise<void>; isTogglingBot?: boolean; toggleBotError?: string | null }) {
  ...
  if (!conversation) return <section className="chat-main flex min-w-0 min-h-0 flex-col bg-slate-100" aria-label="Cửa sổ chat"><div className="flex min-h-0 flex-1"><div className="flex min-w-0 flex-1 flex-col"><div className="chat-empty-state grid h-full place-content-center justify-items-center p-6 text-center"><div className="chat-empty-icon grid size-[120px] place-items-center rounded-full bg-white text-blue-600 shadow-sm"><InboxIcon name="chat" size={52} /></div>
  <p className="mt-3.5 text-base text-gray-500">Chọn một hội thoại từ danh sách bên trái để bắt đầu.</p><button className="mt-5 inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-300 min-[900px]:hidden" type="button" onClick={onOpenConversationList} aria-label="Mở danh sách hội thoại"><InboxIcon name="list" size={17} />Mở danh sách hội thoại</button></div></div><ConversationInfoSidebar hasConversation={false} /></div></section>;
