    ...
    <div className="flex min-h-0 flex-1">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div ref={messagesRef} onScroll={handleScroll} className="chat-messages relative min-h-0 flex-1 overflow-y-auto bg-[#f0f2f5] p-6">
          {isLoadingMessages ? <div className="grid h-full place-items-center" role="status" aria-label="Đang tải tin nhắn"><div className="flex items-center gap-1.5 rounded-2xl border border-blue-100 bg-white px-5 py-4 shadow-sm"><span className="size-2.5 animate-bounce rounded-full bg-blue-500 [animation-delay:-0.3s]" /><span className="size-2.5 animate-bounce rounded-full bg-blue-500 [animation-delay:-0.15s]" /><span className="size-2.5 animate-bounce rounded-full bg-blue-500" /></div></div> : messages.length === 0 ? <p className="chat-no-messages text-center text-gray-400">Chưa có tin nhắn</p> : messages.map((message) => {
