...
function MessageDeliveryIndicator({ message, onRetry }: { message: ChatMessageContract; onRetry?: (messageId: string) => void }) {
  ...
  return <span className={`${position} grid size-5 place-items-center  text-emerald-600 shadow-sm`} aria-label="Đã gửi tin nhắn"><span className="text-xs font-bold">✓</span></span>;
}
...
function PinnedMessagesBar({ pinnedMessages, activeIndex, onChangeIndex, onUnpinMessage, pinError }: { pinnedMessages: PinnedMessageContract[]; activeIndex: number; onChangeIndex: (index: number) => void; onUnpinMessage: MessagePinMutation; pinError?: string | null }) {
  ...
  return <div className="shrink-0 bg-[#f0f2f5] px-4 pt-3">{activePin && <><section className="group/pinned relative flex min-w-0 items-center gap-3 rounded-xl border border-gray-100 bg-white px-3 py-2.5 shadow-sm" aria-label="Tin nhắn đã ghim"><span className="grid size-9 shrink-0 place-items-center rounded-full bg-gray-100 text-gray-600"><InboxIcon name="pin" size={17} /></span><button className="min-w-0 flex-1 pr-8 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-300" type="button" onClick={() => scrollToPinnedMessage(activePin.messageId)} aria-label={`Cuộn tới tin đã ghim ${activeIndex + 1}`}><span className="block text-xs font-semibold text-gray-800">Tin đã ghim · {pinnedMessages.length}/10</span><span className="mt-0.5 block truncate text-xs text-gray-500">{pinnedMessageQuote(activePin)}</span></button><div className="flex shrink-0 items-center gap-1"><button className="invisible grid size-7 place-items-center rounded-full text-gray-400 transition hover:bg-gray-100 hover:text-gray-700 focus-visible:visible group-hover/pinned:visible group-focus-within/pinned:visible" type="button" onClick={(event) => { event.stopPropagation(); setIsPinnedListOpen((current) => togglePinnedMessagesList(current)); }} aria-expanded={isPinnedListOpen} aria-label={isPinnedListOpen ? "Thu danh sách tin ghim" : "Mở danh sách tin ghim"} title={isPinnedListOpen ? "Thu danh sách tin ghim" : "Mở danh sách tin ghim"}><InboxIcon name={isPinnedListOpen ? "chevron-up" : "chevron-down"} size={16} /></button><button className="invisible absolute right-3 grid size-7 place-items-center rounded-full text-gray-400 transition hover:bg-red-50 hover:text-red-600 focus-visible:visible group-hover/pinned:visible group-focus-within/pinned:visible" type="button" onClick={(event) => handlePinnedBarUnpin(event, activePin.messageId, onUnpinMessage)} aria-label="Bỏ ghim tin nhắn" title="Bỏ ghim"><InboxIcon name="close" size={15} /></button></div></section>{isPinnedListOpen && <div className="mt-1 overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm" role="list" aria-label="Danh sách tin nhắn đã ghim">{pinnedMessages.map((pin, index) => <div className="group/pinned-item flex min-w-0 items-center gap-2 border-b border-gray-100 px-3 py-2 last:border-b-0" key={pin.messageId} role="listitem"><button className="flex min-w-0 flex-1 items-center gap-2 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-300" type="button" onClick={() => { onChangeIndex(index); setIsPinnedListOpen(false); scrollToPinnedMessage(pin.messageId); }} aria-label={`Cuộn tới tin đã ghim ${index + 1}`}><span className="grid size-7 shrink-0 place-items-center rounded-full bg-gray-100 text-gray-500"><InboxIcon name="pin" size={14} /></span><span className="min-w-0 truncate text-xs text-gray-600">{pinnedMessageQuote(pin)}</span></button><div className="invisible flex shrink-0 items-center gap-1 opacity-0 transition-opacity group-hover/pinned-item:visible group-hover/pinned-item:opacity-100 group-focus-within/pinned-item:visible group-focus-within/pinned-item:opacity-100"><button className="grid size-7 place-items-center rounded-full text-gray-400 hover:bg-red-50 hover:text-red-600" type="button" onClick={(event) => handlePinnedBarUnpin(event, pin.messageId, onUnpinMessage)} aria-label="Bỏ ghim" title="Bỏ ghim"><InboxIcon name="close" size={14} /></button><button className="grid size-7 place-items-center rounded-full text-gray-400 hover:bg-gray-100 hover:text-gray-700" type="button" onClick={(event) => { event.stopPropagation(); void copyPinnedMessage(pinnedMessageQuote(pin)); }} aria-label="Sao chép tin đã ghim" title="Sao chép"><InboxIcon name="copy" size={14} /></button></div></div>)}</div>}</>}{pinError && <p className="mt-1 text-xs text-red-600" role="alert">{pinError}</p>}</div>;
}

export function ChatWindow({ conversation, messages, onSend, onRetryMessage, quickReplies, pinnedMessages, isPinned, onPinMessage, onUnpinMessage, pinError, onOpenConversationList, isCustomerTyping = false, aiSuggestions, isAiSuggestionsLoading = false, aiSuggestionsError, onRefreshAiSuggestions, aiSuggestionsEnabled = true, availableTags = [], onTagsChange, draft = "", onDraftChange, onToggleBot, isTogglingBot = false, toggleBotError }: ChatWindowProps) {
  ...
  return <section className="chat-main flex min-w-0 min-h-0 flex-col bg-slate-100" aria-label="Cửa sổ chat">
    <header className="chat-header flex min-h-[72px] items-center gap-3 border-b border-gray-200 bg-white px-6 py-3.5">
      ...
      <div><h2 className="mb-1 text-base font-semibold">{name}</h2><span className="inline-flex items-center" title={conversationPlatformLabel(conversation.platform)} aria-label={conversationPlatformLabel(conversation.platform)}><PlatformIcon provider={platformIconProvider} size={15} /></span></div>
      {onToggleBot && <div className="ml-auto flex items-center gap-2">{toggleBotError && <span className="text-xs text-red-600" role="alert">{toggleBotError}</span>}<span className="text-xs font-medium text-gray-500">Bot tự động</span><button className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full p-0 transition-colors ${botEnabled ? "bg-sky-500" : "bg-gray-300"} disabled:cursor-not-allowed disabled:opacity-60 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-blue-300`} type="button" role="switch" aria-checked={botEnabled} aria-label={botEnabled ? "Tắt bot tự động" : "Bật bot tự động"} onClick={() => void onToggleBot()} disabled={isTogglingBot}><span className={`absolute left-1 top-1 size-4 rounded-full bg-white shadow-sm transition-transform ${botEnabled ? "translate-x-5" : "translate-x-0"}`} /></button></div>}
      <button className="chat-header-action ml-auto hidden place-items-center text-gray-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-blue-300 max-[999px]:grid" type="button" onClick={() => setIsInfoSidebarOpen(true)} aria-label="Thông tin hội thoại"><InboxIcon name="users" /></button>
    </header>
    <div className="flex min-h-0 flex-1">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div ref={messagesRef} onScroll={handleScroll} className="chat-messages relative min-h-0 flex-1 overflow-y-auto bg-[#f0f2f5] p-6">
          {messages.length === 0 ? <p className="chat-no-messages text-center text-gray-400">Chưa có tin nhắn</p> : messages.map((message) => {
                ...
                <div className={`flex min-w-0 items-end gap-2 ${message.senderType === "customer" ? "flex-row" : "flex-row-reverse"}`}>
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1 text-right">
                    <div className="flex items-center gap-1 text-gray-500 opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-within:opacity-100">
                      {messageIsPinned
                        ? <button className="grid size-7 place-items-center rounded-full transition hover:bg-red-50 hover:text-red-600" type="button" onClick={() => void onUnpinMessage(message.id)} aria-label="Bỏ ghim" title="Bỏ ghim"><InboxIcon name="pin" size={15} /></button>
                        : <button className="grid size-7 place-items-center rounded-full transition hover:bg-gray-200 disabled:cursor-not-allowed disabled:opacity-40" type="button" onClick={() => void onPinMessage(message.id)} aria-label="Ghim tin nhắn" title={pinLimitReached ? "Đã đạt giới hạn 10 tin ghim" : "Ghim tin nhắn"} disabled={pinLimitReached}><InboxIcon name="pin" size={15} /></button>}
                    </div>
