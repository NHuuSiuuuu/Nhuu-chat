...
function renderMessageAttachments(message: ChatMessageContract): React.ReactNode {
  return message.attachments?.map((attachment) => attachment.mimeType.startsWith("image/")
    ...
    : <div className="relative mb-2" key={attachment.url}><a className="flex items-center gap-2 rounded-lg border border-sky-200 text-gray-900  px-3 py-2 text-gray-900underline" href={attachment.url} target="_blank" rel="noreferrer" aria-label={`Tải tệp đính kèm ${attachment.fileName ?? ""}`}><InboxIcon name="file" size={17} /><span className="min-w-0 truncate">{attachment.fileName ?? "Tệp đính kèm"}</span></a></div>);
}
