...
export function ChatWindow({ conversation, messages, isLoadingMessages = false, onSend, onRetryMessage, quickReplies, pinnedMessages, isPinned, onPinMessage, onUnpinMessage, pinError, onOpenConversationList, isCustomerTyping = false, aiSuggestions, isAiSuggestionsLoading = false, aiSuggestionsError, onRefreshAiSuggestions, aiSuggestionsEnabled = true, availableTags = [], onTagsChange, draft = "", onDraftChange, onToggleBot, isTogglingBot = false, toggleBotError }: ChatWindowProps) {
  ...
  return <section className="chat-main flex h-full min-w-0 min-h-0 flex-col bg-slate-100" aria-label="Cửa sổ chat">
    <header className="chat-header flex min-h-[72px] items-center gap-3 border-b border-gray-200 bg-white px-6 py-3.5 max-[899px]:gap-2 max-[899px]:px-2">
      ...
      <div className="ml-auto flex shrink-0 items-center gap-2 max-[899px]:gap-1">
        ...
        <button className="chat-header-action hidden place-items-center text-gray-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:ring-2 focus-visible:ring-blue-300 max-[999px]:grid" type="button" onClick={() => setIsInfoSidebarOpen(true)} aria-label="Tùy chọn hội thoại"><InboxIcon name="more" /></button>
      </div>
    </header>
    <div className="flex min-h-0 flex-1">
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div ref={messagesRef} onScroll={handleScroll} className="chat-messages relative min-h-0 flex-1 overflow-y-auto bg-[#f0f2f5] p-6">
          {isLoadingMessages ? <div className="grid h-full place-items-center" role="status" aria-label="Đang tải tin nhắn"><div className="grid size-8 place-items-center rounded-full border border-blue-100 bg-white shadow-sm"><div className="flex items-center gap-0.5"><span className="size-1.5 animate-bounce rounded-full bg-blue-500 [animation-delay:-0.3s]" /><span className="size-1.5 animate-bounce rounded-full bg-blue-500 [animation-delay:-0.15s]" /><span className="size-1.5 animate-bounce rounded-full bg-blue-500" /></div></div></div> : messages.length === 0 ? <p className="chat-no-messages text-center text-gray-400">Chưa có tin nhắn</p> : messages.map((message) => {
