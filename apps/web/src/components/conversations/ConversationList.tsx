  ...
  return <aside ref={sidebarRef} className={`relative rounded-tl-none conversation-sidebar flex h-full min-w-0 flex-col overflow-hidden border-r border-gray-200 bg-white transition-[width] duration-200 ${collapsed ? "w-[72px]" : "w-full"}`} aria-label="Danh sách hội thoại" aria-busy={isLoading}>
    ...
    {!collapsed && (isSelectionMode ? <div className="conversation-list-bulk-toolbar flex min-h-11 items-center justify-between gap-2 border-y border-gray-200 px-3 text-xs" aria-label="Thao tác hàng loạt">
      ...
      <div className="flex items-center gap-1">
        ...
        <button className="grid size-8 place-items-center rounded-md text-gray-500 hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-40 cursor-pointer" type="button" aria-label="Thoát chế độ chọn" onClick={closeSelectionMode} disabled={isBulkActionPending}><InboxIcon name="close" /></button>
      </div>
