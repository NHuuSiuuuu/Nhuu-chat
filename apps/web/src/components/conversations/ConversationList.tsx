        ...
        return <div className={`conversation-item relative flex min-h-[88px] w-full border-b border-gray-100 text-gray-800 transition-colors hover:bg-slate-50 ${item.id === activeId ? "bg-blue-50" : "bg-white"}`} key={item.id}>
          <button className={`flex min-w-0 flex-1 items-start gap-3 border-0 bg-transparent text-left focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-300 ${collapsed ? "justify-center px-2 py-3" : "px-3 py-3.5"}`} type="button" onClick={() => onSelect(item.id)} aria-current={item.id === activeId} title={collapsed ? name : undefined}>
            ...
            <span className={`conversation-item-content grid min-w-0 flex-1 gap-1 ${collapsed ? "hidden" : ""}`}>
              ...
              <span className="conversation-account flex min-w-0 items-center gap-1.5 leading-4 text-[11px] text-gray-400">{hasExplicitAccountName ? <span className="flex min-w-0 shrink-0 items-center gap-1.5"><ConversationAvatar name={accountName} avatarUrl={item.accountAvatarUrl} size="size-4" /><span className="min-w-0 max-w-[120px] truncate leading-4">{accountName}</span></span> : <span className="flex min-w-0 shrink-0 items-center gap-1.5" title={conversationPlatformLabel(item.platform)}><PlatformIcon provider={platform} size={16} plain /><span className="leading-4">{conversationPlatformLabel(item.platform)}</span></span>}{tags.length > 0 ? <span className="relative flex min-w-0 flex-1 items-center gap-1.5"><span className="flex min-w-0 flex-1 items-center gap-1.5 overflow-hidden whitespace-nowrap">{visibleTags.map((tag) => <span className="conversation-tag shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold text-white" style={{ backgroundColor: tag.color }} key={tag.id}>{tag.name}</span>)}</span>{hiddenCount > 0 && <span className="group relative shrink-0"><span className="inline-flex rounded-full bg-white px-2 py-0.5 text-[10px] font-bold text-blue-500 shadow-sm ring-1 ring-blue-100" title={`Xem ${hiddenCount} thẻ ẩn`}>+{hiddenCount}</span><span className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 hidden -translate-x-1/2 gap-1 whitespace-nowrap rounded-lg bg-white p-2 shadow-lg ring-1 ring-gray-200 group-hover:flex group-focus-within:flex" role="tooltip">{hiddenTags.map((tag) => <span className="rounded-full px-2 py-0.5 text-[10px] font-bold text-white" style={{ backgroundColor: tag.color }} key={tag.id}>{tag.name}</span>)}</span></span>}</span> : null}{hasExplicitAccountName && <span className="ml-auto shrink-0" title={conversationPlatformLabel(item.platform)}><PlatformIcon provider={platform} size={15} plain /></span>}</span>
            </span>
          </button>
        </div>;
      })}
