...
export function ConnectModal({ token, refresh, onClose, onConnected }: { token: string; refresh?: () => Promise<string | null>; onClose: () => void; onConnected: () => void }) {
  ...
  return <div className={`fixed inset-0 z-20 grid place-items-center bg-slate-900/50 p-3 backdrop-blur-[5px] transition-opacity duration-200 motion-reduce:transition-none sm:p-6 ${closing ? "opacity-0" : "opacity-100"}`} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) requestClose(); }}>
    <section className={`w-full max-w-[1120px] overflow-hidden rounded-[20px] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.28)] transition-[opacity,transform] duration-200 ease-out motion-reduce:transition-none ${closing ? "translate-y-2 scale-[.98] opacity-0" : "translate-y-0 scale-100 opacity-100"}`} role="dialog" aria-modal="true" aria-labelledby="connect-modal-title">
      <header className="flex items-center justify-between border-b border-slate-200/80 px-5 py-[18px] sm:px-7 sm:py-[22px]"><h2 id="connect-modal-title" className="m-0 text-[15px] font-bold text-slate-700 sm:text-xl">Thêm kết nối</h2><button className="cursor-pointer border-0 bg-transparent text-[25px] leading-none text-slate-400 transition-colors hover:text-slate-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-500" type="button" aria-label="Đóng" onClick={requestClose}>×</button></header>
      <div className="grid min-h-[540px] grid-cols-1 sm:grid-cols-[260px_minmax(0,1fr)]">
