    ...
    <section ref={dialogRef} className={`flex max-h-[calc(100vh-2rem)] w-full max-w-[1120px] flex-col overflow-hidden rounded-[20px] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.28)] transition-[opacity,transform] duration-200 ease-out motion-reduce:transition-none ${closing ? "translate-y-2 scale-[.98] opacity-0" : "translate-y-0 scale-100 opacity-100"}`} role="dialog" aria-modal="true" aria-labelledby="connect-modal-title">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto"><div className="grid min-h-[540px] grid-cols-1 sm:grid-cols-[260px_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          {selected === "telegram" ? <TelegramConnectContent error={error} image={image} loading={loading} onStart={() => void startTelegram()} onSubmit={submitPassword} password={password} qr={qr} setPassword={setPassword} /> : selected === "zalo" ? <ZaloConnectContent error={error} image={zaloImage} loading={loading} onStart={() => void startZalo()} qr={zaloQr} /> : <div className="px-9 py-[70px] text-center"><span className="mx-auto mb-[18px] grid h-[54px] w-[54px] place-items-center rounded-full bg-blue-50 text-[28px] text-sky-500">◷</span><h3 className="m-0 text-[17px] font-bold text-slate-800">{provider.label} đang chờ kích hoạt</h3><p className="text-[13px] text-slate-500">Tích hợp kênh này sẽ được bổ sung trong phiên bản tiếp theo.</p></div>}
        </div></div>
      </div>
    </section>
  </div>;
...
function ZaloConnectContent({ qr, image, error, loading, onStart }: { qr: ZaloQrStatus | null; image: string | null; error: string | null; loading: boolean; onStart: () => void }) {
  ...
  return <div className="grid grid-cols-1 items-center gap-6 px-5 py-7 sm:grid-cols-2 sm:gap-12 sm:px-14 sm:py-[60px]"><div>{image ? <figure className="m-0 text-center"><img className="mx-auto block w-[280px] max-w-full" src={image} alt="Mã QR đăng nhập Zalo" /><figcaption className="mt-2.5 text-[11px] text-slate-400">Quét mã QR bằng ứng dụng Zalo</figcaption></figure> : <div className="grid min-h-[190px] place-items-center rounded-xl border border-dashed border-slate-300 text-center text-slate-400 sm:min-h-[260px]"><span className="animate-pulse text-[72px] text-sky-500 motion-reduce:animate-none">▦</span><p className="m-0 text-xs">{loading ? "Đang tạo mã QR..." : "Mã QR sẽ hiển thị tại đây"}</p></div>}</div><div><h3 className="m-0 mb-6 text-xl font-bold text-slate-800">Quét QR để kết nối Zalo</h3><ol className="m-0 list-decimal space-y-1 pl-5 text-[15px] leading-[2.4] text-slate-600"><li>Mở ứng dụng Zalo trên điện thoại</li><li>Chọn <strong className="text-slate-700">Quét mã QR</strong></li><li>Quét mã để kết nối tài khoản</li></ol>{!image && <button className="mt-5 rounded-lg border border-sky-500 bg-sky-500 px-4 py-2.5 text-xs font-bold text-white disabled:cursor-wait disabled:opacity-60" type="button" onClick={onStart} disabled={loading}>{loading ? "Đang tạo mã QR..." : "Tạo mã QR"}</button>}{image && <p className="mt-5 text-sm text-slate-500">Đang chờ quét mã...</p>}{error && <p className="text-xs text-rose-600" role="alert">{error}</p>}</div></div>;
}
