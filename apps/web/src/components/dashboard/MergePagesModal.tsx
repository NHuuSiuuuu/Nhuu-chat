  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="flex max-h-[min(680px,calc(100vh-2rem))] w-full max-w-[520px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="merge-pages-title" onMouseDown={(event) => event.stopPropagation()}>
      <header className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
        ...
        <button className="grid size-9 place-items-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-400 cursor-pointer" type="button" aria-label="Đóng" onClick={onClose}><InboxIcon name="close" size={19} /></button>
      </header>
      <div className="grid gap-4 px-5 py-4">
        ...
        <div className="flex items-center justify-between gap-3 text-sm"><span className="text-slate-500">Đã chọn: <strong className="text-slate-800">{selectedIds.size}</strong></span><button className="font-semiboldtext-gray-900  hover:text-gray-900focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-400 cursor-pointer" type="button" onClick={onSelectAll}>Chọn tất cả</button></div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-4" aria-label="Danh sách Page">
        <div className="grid gap-2">
          {visiblePages.map((page) => {
            const isSelected = selectedIds.has(page.id);
            return <button className={`flex w-full items-center gap-3 rounded-xl border px-3 py-3 text-left transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-400 ${isSelected ? "border-sky-300 text-gray-900 " : "border-slate-200 hover:border-sky-200 hover:bg-slate-50"} cursor-pointer`} type="button" key={page.id} aria-pressed={isSelected} onClick={() => onTogglePage(page.id)}>
      ...
      <footer className="flex items-center justify-end gap-2 border-t border-slate-100 px-5 py-4">
        <button className="rounded-lg px-4 py-2.5 text-sm font-semibold text-slate-500 transition hover:bg-slate-50 hover:text-slate-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-slate-400 cursor-pointer" type="button" onClick={onClose}>Đóng</button>
        <button className="rounded-lg bg-emerald-500 px-4 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-400 disabled:cursor-not-allowed disabled:opacity-50 cursor-pointer" type="button" disabled={selectedIds.size === 0} onClick={onMerge}>Gộp Page</button>
      </footer>
    </section>
  </div>;
}
