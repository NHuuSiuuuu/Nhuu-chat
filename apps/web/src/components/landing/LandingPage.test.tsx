...
describe("LandingPage", () => {
  ...
  it("exposes an accessible dashboard button for an authenticated user", () => {
    ...
    const userButton = renderer.root.findByProps({ "aria-label": "Mở Dashboard cho Nguyễn Ngọc Hữu" });
    act(() => userButton.props.onClick());

    expect(userButton.findAllByType("span").map((span) => span.children.join(""))).toContain("Nguyễn Ngọc Hữu");
    expect(userButton.findAllByType("span").map((span) => span.children.join("")).join(" ")).not.toContain("admin@example.com");
  ...
  it("moves authenticated logout from the mobile header to the end of the mobile menu", () => {
    ...
    const dashboardButton = renderer.root.findByProps({ "aria-label": "Mở Dashboard cho Nguyễn Ngọc Hữu" });
    ...
    expect(mobileNavigation.findAllByType("span").map((span) => span.children.join("")).join(" ")).not.toContain("admin@example.com");
    expect(mobileUser.findAllByType("span")).toHaveLength(2);
